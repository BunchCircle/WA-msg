...
async function renderDesignBlock(
  ...
  const bulletWrapped: string[][] = copy.bullets.map((b) =>
    wrapText(tempCtx, b, TEXT_AREA_WIDTH - 40),
  );
  const bulletLineH = 36;
  const bulletGap = 14;
  const bulletsBlockH = bulletWrapped.reduce(
    (sum, bLines) => sum + bLines.length * bulletLineH + bulletGap,
    0,
  );
...
function buildDesignBlockSvg(
  ...
  const bulletGap = 14;
  const bulletsBlockH = bulletWrapped.reduce(
    (s, bl) => s + bl.length * bulletLineH + bulletGap,
    0,
  );
...
const BusinessQrGenerator: React.FC<BusinessQrGeneratorProps> = ({
  ...
  const generateQr = useCallback(async () => {
    ...
    const errorCorrectionLevel = logoFile
      ? "H"
      : contrastRatio < 4.5
        ? "H"
        : "M";
  ...
  const fetchAiCopywriting = async (
    finalUrl: string,
    qrPngDataUrl: string,
    svgStr: string,
  ) => {
...
{"title": "Scan to Connect or Call Us Directly", "bullets": ["Get instant support at 9958929886", "Find our store location easily", "Browse our latest offerings online"]}`;
  ...
  const getDefaultCopywriting = (): AiCopy => {
    switch (purpose) {
      case "Website / Landing Page":
        return {
          title: "Visit Our Website — Scan to Explore",
          bullets: [
            "Discover our latest products and services",
            "Read customer stories and reviews",
            "Get exclusive online-only offers",
          ],
        };
      case "Store Location (Maps)":
        return {
          title: "Find Us Easily — Scan for Directions",
          bullets: [
            "Opens directly in Google Maps",
            "Get turn-by-turn navigation",
            "See our hours and parking info",
          ],
        };
      case "Reviews":
        return {
          title: "Love Our Service? Share Your Feedback",
          bullets: [
            "Takes less than 30 seconds",
            "Help others discover us",
            "We read and value every review",
          ],
        };
      case "Social Profile":
        return {
          title: "Follow Us — Scan to Connect",
          bullets: [
      ...
      case "Payment Link":
        return {
          title: "Quick & Easy Payment — Scan to Pay",
          bullets: [
            "Secure and instant processing",
            "Multiple payment methods supported",
            "Get a digital receipt instantly",
          ],
        };
      case "Menu / Catalog":
        return {
          title: "Browse Our Full Menu — Scan Here",
          bullets: [
            "See all items with photos and prices",
            "Check daily specials and new arrivals",
            "Dietary info and allergen details included",
          ],
        };
      case "Product Info":
        return {
          title: "Learn More About This Product",
          bullets: [
      ...
      case "Call / Contact":
        return {
          title: "Get in Touch — Scan to Call Us",
          bullets: [
            "Connect directly with our team",
            "No typing needed, one-tap call",
            "Available during business hours",
          ],
        };
      default:
        return {
          title: "Scan to Discover More",
          bullets: [
            "Quick and easy — just point your camera",
  ...
  const renderFullDesignBlock = async (
    qrPngDataUrl: string,
    copy: AiCopy,
    _svgStr: string,
  ) => {
    try {
      const blockCanvas = await renderDesignBlock(
        qrPngDataUrl,
        copy,
        brandColor,
        useGradient ? brandColor2 : null,
        qrStyle,
      );
  ...
  const downloadSvg = useCallback(() => {
    ...
    const svgContent = buildDesignBlockSvg(
      qrSvgString,
      aiCopy,
      brandColor,
      useGradient ? brandColor2 : null,
  ...
  const downloadPdf = useCallback(async () => {
    ...
    const totalSize =
      headerBytes.length +
      objBytes.reduce((s, b) => s + b.length, 0) +
      jpegArray.length +
      imgFooterBytes.length +
      xrefBytes.length;
    const pdfBuffer = new Uint8Array(totalSize);
    let offset = 0;
    pdfBuffer.set(headerBytes, offset);
    offset += headerBytes.length;
    for (let i = 0; i < objBytes.length; i++) {
      pdfBuffer.set(objBytes[i], offset);
      offset += objBytes[i].length;
      if (i === objBytes.length - 1) {
        pdfBuffer.set(jpegArray, offset);
        offset += jpegArray.length;
        pdfBuffer.set(imgFooterBytes, offset);
        offset += imgFooterBytes.length;
      }
    }
    ...
    >
      ...
      <motion.section variants={fadeInUp} className="text-center mb-16 pt-4">
        ...
        <p className="text-base md:text-lg text-slate-500 font-medium max-w-2xl mx-auto">
          Free, fast, and print-ready — QR code + persuasive copywriting in one
          design block
        </p>
      </motion.section>
      ...
      <motion.section
        variants={fadeInUp}
        className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-20 lg:items-start"
      >
        {/* Form Panel */}
          ...
          <div className="glass p-8 md:p-10 rounded-[2.5rem] shadow-2xl space-y-7">
            ...
            <div className="relative">
              ...
              >
                ...
                <ChevronDown
                  size={18}
                  className={`text-slate-400 transition-transform duration-300 ${isPurposeOpen ? "rotate-180" : ""}`}
                />
              </button>
              <AnimatePresence>
                {isPurposeOpen && (
            ...
            <div>
              ...
              {urlError && (
                <motion.p
                  ...
                  className="mt-2 text-sm text-red-500 font-semibold flex items-center gap-1.5"
                >
              ...
              {logoPreview ? (
                <div className="flex items-center gap-4 p-4 rounded-2xl bg-black/5 border-2 border-emerald-200">
                  <img
                    src={logoPreview}
                    alt="Logo preview"
                    className="w-14 h-14 rounded-xl object-contain bg-white shadow-sm"
                  />
                  <div className="flex-1">
                    <p className="font-bold text-sm">{logoFile?.name}</p>
                    <p className="text-xs text-slate-400">
                      Will be centered inside QR code
                    </p>
                  </div>
                  <button
                    onClick={removeLogo}
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      PNG or JPG — auto-resized safely
                    </p>
                  </div>
                </label>
              )}
            ...
            <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <input
                          ...
                          value={brandColor2}
                          onChange={(e) =>
                            handleColor2PickerChange(e.target.value)
                          }
                          className="w-14 h-14 rounded-2xl cursor-pointer border-2 border-slate-200 hover:border-emerald-400 transition-all appearance-none bg-transparent qr-color-picker"
                        />
                      </div>
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          value={color2Input}
                          onChange={(e) =>
                            handleColor2InputChange(e.target.value)
                          }
                          placeholder="#4f46e5"
                          maxLength={7}
                          className="w-full pl-11 pr-4 py-4 rounded-2xl bg-black/5 border-2 border-slate-200 focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/5 transition-all font-mono font-bold text-lg outline-none"
              ...
              {getContrastRatio(brandColor, "#ffffff") < 3 && (
                <motion.p
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="mt-2 text-sm text-amber-600 font-semibold flex items-center gap-1.5"
                >
                  <AlertCircle size={14} /> Low contrast — QR may be hard to
                  scan
                </motion.p>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-sm font-medium text-slate-400 mb-2 ml-1">
                AI will generate a persuasive tagline to print alongside your QR
                code
              </p>
              <div className="relative">
                <MessageSquareText
                ...
                <textarea
                  ...
                  placeholder={
                    '"For restaurant table stickers" or "For product packaging" or "Discount offer for walk-in customers"'
                  }
                  rows={3}
                  maxLength={200}
                ...
                />
                <span className="absolute bottom-3 right-4 text-xs text-slate-300 font-bold">
                  {aiInstruction.length}/200
                </span>
              </div>
            </div>
        ...
        <div className="lg:col-span-5">
          <div className="glass p-8 md:p-10 rounded-[2.5rem] shadow-2xl sticky top-28 space-y-6">
            ...
            <div
              ref={designBlockRef}
              className="bg-white rounded-3xl shadow-inner border-4 border-slate-50 overflow-hidden"
            >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-300 mt-1">
                        Your print-ready design block will appear here
                      </p>
                    </div>
                  </motion.div>
                )}
            ...
            <AnimatePresence>
              {aiCopy && (
                ...
                >
                  ...
                  <p className="text-sm text-slate-800 font-bold leading-relaxed mb-1.5">
                    "{aiCopy.title}"
                  </p>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 justify-center pt-1">
                    ...
                    <span className="text-[10px] text-slate-400 font-medium">
                      Print-ready for posters, banners, table tents & more
                    </span>
                  </div>
                </motion.div>
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="flex flex-col items-start md:items-end gap-4">
            ...
            <p className="text-xs text-slate-400 font-medium">
              No spam. We'll only notify you at launch.
            </p>
          </div>
        </div>
      </motion.section>
